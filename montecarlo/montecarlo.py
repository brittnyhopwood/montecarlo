...
class Analyzer():
    ...
    def compute_combo(self, times):
        ...
        for game in self.games:
            df = game.play(times)
            combo_df = df.apply(lambda row: tuple(sorted(row)), axis=1).value_counts().reset_index()
    ...
    def compute_jackpot(self, times):
        '''
        PURPOSE: This method computes the number of times the game resulted in all faces being identical. 
        ...
        '''
        self.compute_combo(times)
        self.jackpot_results_df = self.combo_df.apply(lambda row: len(set(row)) == 1, axis=1)
        jackpot_df = pd.DataFrame({'jackpot_count': self.jackpot_results_df.sum()}, index=range(1, times+1))
        jackpot_df.index.name = 'roll_number'  # Set the index name
        return jackpot_df.reset_index()

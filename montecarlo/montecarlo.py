...
class Analyzer():
    ...
    def compute_combo(self, times):
        ...
        for game in self.games:
            df = game.play(times)
            combo_df = df['face_rolled'].value_counts().reset_index()
            combo_df.columns = ['face_rolled', 'count']
    ...
    '''
        ...
        for _ in range(times):
            df = self.game.play(1)  # Play 1 round
            combo_df = df['face_rolled'].value_counts()
            if len(combo_df) == 1:
                jackpot_count += 1
